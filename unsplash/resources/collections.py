...
class CollectionsResource:
    ...
    def list(self, page: int = 1, per_page: int = 10) -> list[Collection]:
        """List all collections."""
        response = self._client.request(
            "GET", "/collections", params={"page": page, "per_page": per_page}
        )
    ...
    def photos(
        ...
        response = self._client.request(
            "GET", f"/collections/{collection_id}/photos", params=params
        )
        return [Photo.model_validate(item) for item in response.json()]

    def related(self, collection_id: str) -> builtins.list[Collection]:
...
class AsyncCollectionsResource:
    ...
    async def list(self, page: int = 1, per_page: int = 10) -> list[Collection]:
        """List all collections."""
    ...
    async def related(self, collection_id: str) -> builtins.list[Collection]:
        """Get related collections."""
        response = await self._client.request(
            "GET", f"/collections/{collection_id}/related"
        )
        return [Collection.model_validate(item) for item in response.json()]
